...
<?php
///////////////////////////////////////////////////////////////////////////////////
//	Name: get-trigger-withtemplate.php
//	Description: Get trigger for template in JSON format.
///////////////////////////////////////////////////////////////////////////////////
...
$log_file = rtrim($log_dir, '/') . "/get-trigger-withtemplate_{$date}.log";
if (!file_exists($log_dir)) {
	mkdir($log_dir, 0755);
}

//Datafile get
$data_dir = rtrim($data_dir, '/') . "/get-trigger-withtemplate";
...
$temp_dir = rtrim($output_dir, '/') . "/get-trigger-withtemplate_{$date}_{$time}";
	...
	if (isset($data_array['groups'][0])) {
		...
		for ($j = 0; $j < $count_groups; ++$j) {
			...
			for ($k = 0; $k < $count_hosts; ++$k) {
				$host_listid = $response['result'][$k]['templateid'];
...
foreach ($export_array as $key => $value) {
	...
	$output_file = $temp_dir . "/" . "get-trigger-withtemplate_{$output_host}_{$date}_{$time}.json";
...
if (isset($output_array[0])) {
	$z_file = "{$output_dir}/get-trigger-withtemplate_{$date}_{$time}.zip";
